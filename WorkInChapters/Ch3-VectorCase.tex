...
\end{align*}
where $n_I$ denotes the vector in $\reals^3$ that is normal to the plane $\clbracs{0}\times I_2\times[0\infty)$ \tstk{remark about how this is one of the F.T'ed planes in our waveguide.}

\tstk{now what happens if we rotate?}
...
By our earlier deduction we know that a tangential $k$-curl has the form $\curl{k}u=\bracs{v_1,0,0}^{\top}$.
