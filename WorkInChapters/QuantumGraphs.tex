...
Rather we will begin with a singular-structure problem over a graph, and demonstrate that it is equivalent to a quantum graph problem, to which we can apply the following theory.
...
The the focus of chapters \ref{ch:ScalarEqns} and \ref{ch:VectorEqns} is to make this statement mathematically precise.
...
\begin{align*}
	...
	\begin{cases}
		-\diff{u_{jk}}{t} = f_{jk} \text{on } \sqbracs{0,l_{jk}}, \\
	...
	\begin{cases}
		-\diff{u_{jk}}{t} = \lambda u_{jk} \ \text{on } \sqbracs{0,l_{jk}}, \\
		u \text{ is continuous at each } v_j\in V, \\
		\sum_{j\con k}\diff{u_{jk}}{t}\bracs{v_j} = 0 \ \forall v_j\in V.
	\end{cases}
\end{align*}
