...
As such we will begin this section by defining several function spaces that we wish to work on, then providing examples of the types of boundary conditions that we might want to consider, before finally providing a concrete example of a differential operator on a quantum graph. \tstk{something like reader is referred to SOME REFS, which is where the information presented here is transcluded from.}
\newline
