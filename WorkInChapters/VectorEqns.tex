...
\tstk{SIM NOTATION NEEDS ADDRESSING!}
...
\end{align*}
In addition to the translation invariance of our domain in the $x_3$-direction (section \ref{sec:PhysicalSystem}) \tstk{this needs to exist at some point!} we will also be taking a Gelfand transform in our cross-section (the $\bracs{x_1,x_2}$-plane).
However the structure in the cross section is a periodic embedded graph, so we obtain a family of problems on the period cell $\graph\subset\ddom:=\bracs{0,1}^2$ parametrised by the quasi-momentum $\qm$.
Denote by $\ddmes$ the singular measure of $\graph$.
...
For the remainder of this section we will also assume our usual set-up; we have some period cell $\ddom$ in which is the period graph $\graph=\bracs{V,E}$ of some larger periodic embedded graph, and we denote by $\ddmes$ the singular measure supported on the edges of $\graph$.
...
\begin{proof}
	...
	%these \con-s are correct as we have fixed v_j
	\begin{align*}
		0 = \integral{\ddom}{u\cdot\overline{\ktgrad_\ddmes v}}{\ddmes}
