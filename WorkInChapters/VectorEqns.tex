...
Finally we round off this section by stating the analogue of \ref{thm:CharGradSob}.
...
\tstk{need to define $U$ again here and swap all the stuff over from $u_1\rightarrow U_1$ and similar for $U_2$s. Will also need to do this for the test function too}
We begin by recalling the form for the tangential $\kt$-curl given by theorem \ref{thm:ktTanCurlsGraphs}, and writing the integral in \eqref{eq:CurlCurlEquationDivFree} as a sum of integrals over the edges of $\graph$;
\begin{align*}
...
\tstk{either here or when we use it in ExampleSystems, we should note that we can effectively think of $U_2$ as being $u_2$ or something to this effect, basically how much do you want to rewrite? :L}
