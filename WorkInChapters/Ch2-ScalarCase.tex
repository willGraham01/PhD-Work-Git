...
%We are now ready to prove that $B\subset\gradset{\ddom}{0}$.
%\begin{prop} \label{prop:BIncGrad0}
%	For $B=\mathset{g\in\ltwo{\ddom}{\ddmes} \setST g\vert_{I_{ij}}\cdot e_{ij}=0 \ \forall I_{ij}\in E}$, we have
%	\begin{align*}
%		B \subset \gradset{\ddom}{0}
%	\end{align*}
%\end{prop}
%\begin{proof}
...
%	Then for each $i,j$ with $i\sim j$, the function $\eta_{i}^{n}\eta_{j}^{n}g\vert_{I_{ij}}$ satisfies the hypothesis of \ref{lem:SegGradExtend}, so $\eta_{i}^{n}\eta_{j}^{n}g\vert_{I_{ij}}\in\gradset{\ddom}{0}$.
%	Furthermore, as $\gradset{\ddom}{\ddmes}$ is a linear subspace of $\ltwo{\ddom}{\ddmes}^{2}$, $g_{n}\in\gradset{\ddom}{0}$ too, for all $n$.
%	By closure of $\gradset{\ddom}{0}$; $g_{n}$ converges in $\gradset{\ddom}{0}$ provided it converges at all, so it remains to show that $g_{n}\lconvN{2} g$ as $n\rightarrow\infty$.
%	However with the result of \ref{lem:etaConv}, we have that $\eta_{i}^{n}\eta_{j}^{n}g\vert_{I_{ij}}\lconvN{2} g\vert_{I_{ij}}$ and hence
%	\begin{align*}
%		g_{n} \lconvN{2} &\recip{2}\sum_{i\in V}\sum_{j\sim i}g\vert_{I_{ij}} = g \text{ as } n\rightarrow\infty,
%	\end{align*}
%	so $g\in\gradset{\ddom}{0}$.
%\end{proof}
