...
\begin{align} \label{eq:5VertexGraphGeneralDet0}
...
\end{align*}
Call the right hand side of this expression $\Xi\bracs{\effFreq}$; and note that the left hand side of this expression is bounded between $-3$ ($\qm=0$) and $\frac{3}{2}$ ($\qm_1=\qm_2=\frac{2\pi}{3}$).
...
\end{figure}
We can observe that for a fixed $\wavenumber$ the points $\omega$ that satisfy \eqref{eq:ExampleThickVertexDispExpr} are divided into distinct ``spectral bands" which shrink as $\omega\rightarrow\infty$. \tstk{can probably prove this too, use the TFR as motivation I guess :L}
...
Importantly the shape of these regions is similar to experimental band-gap plots in a photonic setting;
...
\begin{itemize}
	\item Cross in plane - how ideas come together
	\item 5 Vertex graph - gaps exist, but even simple setups are hard to treat analytically. Reiterate need for numerical stuff
	\item Thick vertex - now in equal-scaling case, useful and interesting for problems we consider. Gaps arise easily and periodically in our simple example. Disp. plot shape agrees well with PCF plots.
\end{itemize}
Numerical scheme discussion, this is an avenue for future research.
In summary though, we have gone from sing-struc to QG to spectrum, and maintained link to thin-struc as well.
Next chapter is just a summary of the report, whilst this one tied together the theory from previous chapters.