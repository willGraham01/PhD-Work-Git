...
NB: Erm... two edges going same way into one vertex in the equivalent QG... gulp!
...
\end{align*}
We also have to compute the rotation matrices $R_{jk}$ for the edges and hence (given $\qm\in[-\pi,\pi)^2$), the parameters $\qm_{jk}$ \tstk{write these down!}.
...
Can include the pretty bandgap plot that I made here :) - might want to remove the colourbar though! Might also want the plot that comes out of the 1D (fixed $\kappa$) case.
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{TFR_Curls_BandgapPlot_alpha-1.pdf}
	\caption{\label{fig:TFR_Curls_BandgapPlot_alpha-1} Dispersion plot for the system in \ref{sec:ExampleThickVertex}, yellow regions correspond to $\omega, \wavenumber$ pairs that solve \eqref{eq:CurlCurlEquationDivFree}. We observe spectral ``band-gaps" in $\omega$ for each $\wavenumber$, and \tstk{maybe compare to physical plot from papers?}}
\end{figure}
