...
\end{proof}
In particular, proposition \ref{prop:CurlOfGradIsZero} informs us that the $\kt$-tangential curl of either a $\kt$-tangential gradient, or a gradient of zero, is zero.

Another remark that deserves to be made is that given the construction of $W^{\kt}_{\rho,\mathrm{grad}}$, it is sufficient to only test orthogonality of $u$ against smooth $\kt$-gradients when checking if a field is divergence free.
