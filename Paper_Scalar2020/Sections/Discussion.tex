...
However provided that one has an appropriate numerical scheme, \tstk{could ref Gissuer from manchester paper here} a computer can compute (an approximation to) the spectrum of \eqref{eq:QGFullSystem} and thus, by varying $\qm$ over a suitable mesh, the spectrum of \eqref{eq:WholeSpaceLaplaceEqn}.
...
We take as a baseline that one has an appropriate numerical scheme for handling generalised eigenvalue problems to hand \tstk{Gissuer would be good here}, and so do not delve into the details of how such an algorithm would operate.
...
\tstk{on a hopeful stability result over $\qm$ to speed up computation}
