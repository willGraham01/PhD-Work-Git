...
In this section we provide an overview of how we obtain the system \eqref{eq:QGFullSystem} from \eqref{eq:PeriodCellLaplaceStrongForm}, and then proceed with a discussion revolving around the methods that can be employed for solving \eqref{eq:QGFullSystem}.
Precise definition and analysis of the ``Sobolev spaces" used here can be found in the appendix (section \ref{ssec:SobSpacesTheory}), but the beginning of section \ref{ssec:Derivation} gives an intuitive overview of this theory, prior to beginning the derivation.
Also, just to reiterate what was said in section \ref{sec:QuantumGraphs}, convention \ref{conv:MeasTheoryProblemSetup} is assumed throughout this section and the rest of this work.

\subsection{Derivation and Formulation of Quantum Graph Problem} \label{ssec:Derivation}
The interested reader can find the definitions and analysis of the Sobolev space $\gradSobQM{\ddom}{\ddmes}$ in the appendix (sections \ref{ssec:MeasureTheory} through \ref{ssec:SobSpacesTheory}), as well as the complete analysis of the ``gradients" $\tgrad_{\ddmes}u$. 
\tstk{We promised an intuitive overview here!}
In this section we give an overview of how we are now able to derive \eqref{eq:QGFullSystem} from \eqref{eq:PeriodCellLaplaceStrongForm}.
