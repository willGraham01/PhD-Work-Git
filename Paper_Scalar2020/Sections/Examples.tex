...
The remaining parts of the spectrum are determined by the equation \tstk{LIES, there is a term missing}
\begin{align*}
...
\end{figure}
We now have to consider the possibility that some solutions that have been found coincide with poles of $H_\qm^{(2)}\bracs{\omega^2}$, which occur when $\omega = 2n\pi, n\in\naturals_0$.
...
\end{align*}
\tstk{numerical results should go here now - also add 0 into the figures that are generated to be consistent? Although they do only plot $\Xi$, so maybe keep them like this and mention in caption that this is only the curve $\Xi$.}
