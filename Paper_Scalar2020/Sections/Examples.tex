...
\end{align}
excluding $\omega=2n\pi, n\in\integers$ \tstk{check this! Meeting notes 22/03/21}.
For ease, we define
\begin{align*}
...
\begin{figure}[t!]
	...
	\begin{subfigure}[t]{0.45\textwidth}
		\centering
		\includegraphics[scale=0.5]{Scalar_CrossInPlane_DoS_alpha-1.pdf}
	...
	\begin{subfigure}[t]{0.45\textwidth}
		\centering
		\includegraphics[scale=0.5]{Scalar_CrossInPlane_DoS_alpha-4.pdf}
...
The value of $\alpha$ affects whether there is a gap in the spectrum between 0 and the first spectral band; values of $\alpha<-2$ result in the first band starting at 0 \tstk{check this! Meeting notes 22/03/21}, whilst $\alpha\geq -2$ results in a gap between zero and the bottom of the spectrum. \tstk{check phrasing here}
This can be contrasted with the work of \cite{cherednichenko2019time}, in which physical constraints on the parameters in the problem prevented the opening of this band-gap at the bottom of the spectrum. \tstk{Can probably be elaborated on}.
