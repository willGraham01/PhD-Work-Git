...
Let $\graph=\bracs{\vertSet,\edgeSet}$ be a finite graph embedded into the region $\ddom=\sqbracs{0,T_1}\times\sqbracs{0,T_2}\subset\reals^2$, which will serve as our singular structure.
...
\end{align*}
In sections \tstk{examples, maybe also discussion} we will consider $\graph$ as the period cell of some periodic, embedded graph in $\reals^2$, and the operator $\tgrad$ arises after using a Gelfand transform to aid us in solving differential equations posed on such a graph.
