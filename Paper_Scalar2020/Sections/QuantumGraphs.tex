...
\tstk{The discussion on GRs and why we can ``actually use" our method}
As discussed in \cite[page 1846]{cherednichenko2018effective}, introducing an explicit dependence on $\omega^2$ in the vertex conditions (or in the domain of $\mathcal{A}$) does not affect the structure of the arguments that underpin the theory in section \ref{ssec:MMatrix}.
\tstk{The paper we cite specifically refers to those arguments found in Ryzhov, V.: Weyl-Titchmarsh function of an abstract boundary value problem, operator colligations, and linear systems with boundary control. Complex Anal. Oper. Theory 3(1), 289–322 (2009)}
Ultimately, the analysis of (the spectrum of) \eqref{eq:QGFullSystem} is carried out by replacing the coupling constants $\alpha_j$ with their $\omega^2-$dependant counterparts, that is by replacing the matrix $\alpha$ with $B = \omega^2\alpha$ in section \ref{ssec:MMatrix}.
An alternative to working with generalised resolvents directly is also highlighted in \cite[Section 6]{cherednichenko2017norm} --- one can look to transform a problem with $\omega^2$-dependent $\delta$-type vertex conditions into a problem with $\omega^2$-independent $\delta'$-type vertex conditions.
Of course, one would have to find the appropriate (unitary) transform, but the theory of section \ref{ssec:MMatrix} would then directly apply to the transformed problem.
...
\tstk{it should be $M+\alpha$ throughout due to the definition of our normal derivs and DN maps, no? Also needs addressing in section 4 and the examples of section 5 if true.
Alternatively, put the $-$ sign into the BCs and change $\nu$ measure to be $-\alpha$ in it, and change accordingly in the derivation and appendices}
It can be shown that $z_0\in\complex$ belongs to the spectrum of $\mathcal{A}$ if and only if $\bracs{M\bracs{z} - \alpha}^{-1}$ does \emph{not} admit analytic continuation into $z_0$ (see \cite[Theorem 2.1]{ershova2014isospectrality}).
Equivalently, zero is an eigenvalue of $M\bracs{z_0}-\alpha$ if and only if $z_0$ is an eigenvalue of $\mathcal{A}$ (as presented in \cite[Proposition 1]{derkach1991generalized}, \cite[page 698]{cherednichenko2019time}).
The eigenvalues of $\mathcal{A}$ can thus be found through analysis of the $M$-matrix, by solving for pairs $u\in\mathrm{dom}\bracs{\mathcal{A}}$, $z_0\in\complex$ such that
\begin{align*}
	\bigl( M\bracs{z_0} - \alpha \bigr) \dmap u = 0.
\end{align*}
