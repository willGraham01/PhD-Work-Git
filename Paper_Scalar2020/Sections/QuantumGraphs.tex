...
Although the concept of an an embedded graph and a singular measure is needed to link \eqref{eq:PeriodCellLaplaceStrongForm} and \eqref{eq:QGFullSystem} (see section \ref{ssec:OurSystem}), these details are left to the appendix \tstk{app ref?} as the focus of this work is analysis and determination of the spectrum of \eqref{eq:QGFullSystem} rather than it's derivation. \newline
...
Throughout this work, we also adopt the following convention:
\begin{convention} \label{conv:MeasTheoryProblemSetup}
	...
	\end{align}
	and let $n_{jk}$ be the unit normal to $I_{jk}$ such that $y_{jk} = \bracs{e_{jk}, n_{jk}}$ can be obtained by an orthonormal rotation $R_{jk}\in\mathrm{SO}(2)$ of the (canonical) axis vectors $x = \bracs{x_1, x_2}$, formally by $x = R_{jk}y_{jk}$.
\end{convention}
Whilst we do not have to make this assumption on our edges, we restrict ourselves to graphs of this class.
The analysis in the appendix (\tstk{ref}) does consider the possibility of including curved edges in the graph, which gives rise to a system of similar form to \eqref{eq:QGFullSystem} only with the parameters $\qm_{jk}$ being $t$-dependant (so non-constant).
...
\tstk{the remainder of this content is to be moved to the appendix!}
...
For each edge $I_{jk}$ we will use the parametrisation in \eqref{eq:EdgeParameterisation} where $e_{jk}$ is the unit vector parallel to $I_{jk}$ and directed from $v_j$ to $v_k$ (consistent with the direction of the edge $I_{jk}$ itself).
