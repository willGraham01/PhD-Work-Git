...
We also assign each vertex $v_j$ a ``coupling constant" $\alpha_j\in\complex$ (although in our examples we will only be considering $\alpha_j<0$, to be consistent with our physical motivation in section \ref{ssec:PhysMot}).
...
\tstk{We don't mention the fact that $\alpha$ should be thought of as a ratio of volumes from a thin-structure problem, nor elaborate on what it should be from physical intuition - we have mentioned we'll only deal with $\alpha_j<0$ but we might want to elaborate here.}
...
Letting $\widetilde{\beta}_j(z), j\in\clbracs{1,...\abs{\vertSet}}$ denote the eigenvalues of the matrix $\widetilde{M}(z) := H^{(1)}\bracs{z} - \bracs{H^{(2)}(z)}^{-1}A$ which are continuous functions of $z$, we can determine those $z$ for which at least one $\widetilde{\beta}_j(z)=0$ by solving
\begin{align*}
