...
\begin{subequations} \label{eq:SignedDerivConvention}
	...
	\end{align}
\end{subequations}
\tstk{when we have QM terms, we need to clarify! Namely, we should have
	\begin{align}
		\bracs{\pdiff{}{n}+ \rmi\qm}u_{jk}\bracs{v_j} &= -\bracs{u_{jk}' + \rmi\qm u_{jk}}\bracs{v_j} = -\lim_{t\rightarrow0}u_{jk}'(t)+\rmi\qm u_{jk}(t), \\
		\bracs{\pdiff{}{n}+ \rmi\qm}u_{jk}\bracs{v_k} &= \bracs{u_{jk}' + \rmi\qm u_{jk}}\bracs{v_k} = \lim_{t\rightarrow l_{jk}}u_{jk}'(t)+\rmi\qm u_{jk}(t),
	\end{align}
for consistency, since we use shifted gradients! (See Kirill-Sasha-CritConstrastPaper) I don't think we explain this notation until it appears in sections 3 and 4, and even then we don't articulate what it means}
...
\tstk{$\alpha_j<0$ is physically relevant cases - mention?}
...
\end{align*}
\tstk{EKK Isospectrality, theorem 2.1, only gives that $z_0$ is in the spectrum if and only if $\bracs{M\bracs{z} - A}^{-1}$ admits analytic continuation to $z_0$. Kirill's Critical Contrast Paper \cite[page 698]{cherednichenko2019time}, on page 698, states the above, but doesn't provide an explicit reference.} where $A = \mathrm{diag}\bracs{\alpha_1, \alpha_2, ... , \alpha_{\abs{\vertSet}}}$ is the diagonal matrix of the vertex coupling constants.
...
Let us denote by $\beta_j(z), j\in\clbracs{1,...\abs{\vertSet}}$ the eigenvalues of the matrix $M(z)-A$ viewed as \tstk{continuous up to a set of discrete points - still have poles in them?} functions of $z$.
...
\end{align*} 
for at least one \tstk{is this enough, or does it need to be ALL?} $\widetilde{\beta}_j$ with $\widetilde{\beta}_j\bracs{z_0}=0$, then $z_0$ is in the spectrum of $\mathcal{A}$, otherwise it is not.
