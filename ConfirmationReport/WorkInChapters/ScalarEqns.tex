...
In this chapter we look at scalar-valued wave-equations in our singular-structure geometry, basing our approach largely off the work of Zhikov \cite{zhikov2000extension}, and consider variational problems with respect to a non-Lebesgue measure $\nu$.
We shall give a formal definition of said variational problems and the associated function spaces, before outlining some of the theoretical results we will make use of.
Then we shall consider a context that is specific to the singular-structure problems discussed in section \ref{sec:VariationalProblemLitReview} and will demonstrate how we can obtain an equivalent quantum graph problem from our original singular-structure problem.
...
The overall goal of this chapter is to highlight the existing ideas and techniques that are available to us, so that we can adapt them for the problems we want to examine (section \ref{sec:OurPhysicalSetup}) more descriptive (or physical), and in particular for the vector-valued systems we consider in chapter \ref{ch:VectorEqns}.

\section{The Scalar Sobolev Spaces} \label{sec:ScalarSobSpaces}
In this section we look to construct the function spaces that are central to our variational problems, as well as discussing some of the consequences of this construction.
...
Let us define the set of ``gradients of zero", $\gradZero{D}{\nu}$, as follows.
\begin{definition}[Set of Gradients of Zero] \label{def:SetGradZeroGeneral}
...
\section{A Graph in a Plane} \label{sec:ScalarExample}
To compliment the theory outlined in section \ref{sec:ScalarSobSpaces}, and to highlight some of the considerations we shall be taking forward, we now provide an example singular-structure problem and some analysis of the spaces and objects involved with said problem.
...
It is worthwhile recapping how the setup just detailed relates to the wave propagation problems that we wish to consider in future (hinted at in section \ref{sec:OurPhysicalSetup}), which is illustrated in figure \ref{fig:ScalarStrucDiagram}.
\begin{figure}[b!]
	\centering
...
\begin{lemma}[Gradients under Rotation] \label{lem:SmoothGradientsUnderRotation}
	...
	Then
	\begin{align*}
		\grad_x \phi &= R \grad_y \phi, \\
		\grad_y \phi &= R^{\top} \grad_x \phi.
	\end{align*}
\end{lemma}
\begin{proof}
	We use index notation to save explicitly writing summation symbols;
	\begin{align*}
		\bracs{\grad_x \phi}_j = \pdiff{\phi}{x_j} 
		= \pdiff{\phi}{y_k}\pdiff{y_k}{x_j}
		= R_{jk}\pdiff{\phi}{y_k} 
		= \bracs{\grad_y \phi}_j.
	\end{align*}
\end{proof}
...
\begin{definition}[Smooth Function Centred on a Vertex, $\eta$] \label{def:etaDef}
...
Having defined a the family of functions $\eta_j^n$ that will be useful when considering vertices of a graph $\graph$, we now define another family of functions that will help us when dealing with the edges of said graph.
...
\end{proof}
Furthermore, we remark that $\abs{\grad\chi_I^\eps}$ is bounded by a constant that depends on $\eps$ only.
\begin{cory}[Bound on $\abs{\grad\chi_I^\eps}$] \label{cory:BoundChiGradient}
...
\begin{proof}
	...
	Setting $\tilde{\phi}_n(t) := \phi_n\bracs{r(t)}$, we have by the chain rule $\diff{\tilde{\phi}_n}{t} = \widetilde{\bracs{\partial_1\phi_n}}$, and so
	\begin{align*}
...
As such $\lambda_I$ can only see the change along the segment $I$, hence we find that $\gradZero{\ddom}{\lambda_I}$ consists of all the gradients that are directed perpendicular to $I$.
We shall shortly show that any gradient orthogonal to $\gradZero{\ddom}{\lambda_I}$ is directed parallel to $I$, to compliment this interpretation.
...
Due to the fact that we begin from an infinite, periodic singular structure and then apply the Gelfand transform to bring us onto a family of singular-structure problems on the unit cell of our domain, in this section we shall study some slightly altered Sobolev spaces from those introduced in section \ref{sec:ScalarSobSpaces}.
We shall then demonstrate how analysis akin to that in section \ref{sec:ScalarExample} is employed to reduce the resulting problems on the period cell to a quantum graph problem in section \ref{sec:ReduceToQMProblem}, which can then be analysed or solved using the methods detailed in chapter \ref{ch:QuantumGraphs}. \newline
...
\begin{definition}[Sobolev Spaces $\gradSobQM{\ddom}{\ddmes}$] \label{def:qmGradSobs}
	Fix $\qm\in[-\pi,\pi)^2$.
...
We see the tangential gradient is related to a gradient (in the sense of distributions) along the segment, with the $\qm_1$ term accounting for the mismatches that the measure $\lambda_I$ can ``see".
...
That being said, a direct consequence of lemma \ref{lem:TangentialQMGradientSegmentParallel}, proposition \ref{prop:GradZeroInvarientUnderQM} and corollary \ref{cory:CharacterisationGradientsZero} is the following result which provides us the form for the tangential $\qm$-gradient.
...
Of particular note is the behaviour of functions $u\in\gradSobQM{\ddom}{\ddmes}$ near the vertices $v_j\in V$, something that is not necessary to be considered when dealing with the gradients of zero.
...
We aim to provide a detailed proof of the following result, which can be found stated in \cite{zhikov2002homogenization}.
We highlight that \cite{zhikov2002homogenization} provides a proof of this result, and what we present below is simply the same argument but expanded in detail so that we can draw ideas from it in later chapters.
...
Throughout these results, we will assume that the conditions
\begin{align*}
...
\begin{proof}
	...
	\end{align*}
	The norms in the first sum existing because of the assumption (i), and using
	\begin{align*}
		\clbracs{0\leq\chi_{jk}^n\leq 1} = \supp\bracs{\grad_{\lambda_{jk}}\chi_{jk}^n}.
	\end{align*}
	It remains to show that the second term is bounded independently of $n$, which in turn implies that $\norm{\grad_{\ddmes}u_n}_{\ltwo{\ddom}{\ddmes}^2}^2$ is bounded. 
...
Furthermore the condition (ii) is important because it ensures that we have enough boundary data when we transition from our variational problem to a quantum graph problem, which is the focus of the next section.
...
\begin{align*}
	...
	&= \integral{\ddom}{\grad_\ddmes^\qm u \cdot \overline{\grad^\qm \phi}}{\ddmes} \quad \forall \phi\in\smooth{\ddom},
\end{align*}
implying that
\begin{align*}
	\omega^2 &\sum_{v_j\in V}\sum_{j\conLeft k}\integral{I_{jk}}{u_{jk}\overline{\phi}}{\ddmes} \\
	&\quad = \sum_{v_j\in V}\sum_{j\conLeft k}\integral{I_{jk}}{R_{jk}^\top \begin{pmatrix} u_{jk}' + i\bracs{R_{jk}\qm}_1 u_{jk} \\ 0 \end{pmatrix} \cdot R_{jk}^\top \overline{\begin{pmatrix} \phi_{jk}' + i\bracs{R_{jk}\qm}_1 \phi_{jk} \\ 0 \end{pmatrix}}}{\lambda_{jk}} \\
	&\quad = \sum_{v_j\in V}\sum_{j\conLeft k}\integral{I_{jk}}{\bracs{u_{jk}' + i\bracs{R_{jk}\qm}_1 u_{jk}}\bracs{\overline{\phi}_{jk}' + i\bracs{R_{jk}\qm}_1 \overline{\phi}_{jk}}}{\lambda_{jk}}. \labelthis\label{eq:EquationReductionSumExpression}
\end{align*}
...
That is
\begin{align*}
	0 &= \integral{I_{jk}}{\bracs{u_{jk}' + i\bracs{R_{jk}\qm}_1 u_{jk}}\bracs{\overline{\phi}_{jk}' + i\bracs{R_{jk}\qm}_1 \overline{\phi}_{jk}} - \omega^2 u_{jk}\overline{\phi}}{\lambda_{jk}}.
\end{align*}
...
We also set $\qm_{jk} := \bracs{R_{jk}\qm}_1$ and obtain
...
We have already found some of these boundary (or rather, ``vertex") conditions in theorem \ref{thm:CharGradSob}, which are the conditions of continuity of $u$ at the vertices, thus providing the matching conditions
\begin{align} \label{eq:EdgeEquationsMatchingConditons}
	u \text{ is continuous at all } v_j\in V.
\end{align}
This still does not provide enough matching conditions for the whole system - to find the remaining conditions we return to \eqref{eq:EquationReductionSumExpression} and note that this equation also holds when the test functions $\phi$ have a support that contains one particular $v_j$ in it's interior (and no other vertices), for each $v_j\in V$.
In this case we observe that
\begin{align*}
	0 &= \sum_{j\con k}\integral{I_{jk}}{\bracs{u_{jk}' + i\bracs{R_{jk}\qm}_1 u_{jk}}\bracs{\overline{\phi}_{jk}' + i\bracs{R_{jk}\qm}_1 \overline{\phi}_{jk}} - \omega^2 u_{jk}\overline{\phi}_{jk}}{\lambda_{jk}} \\
...
The tangential gradient fulfils the role that the classical gradient would otherwise, and is found by characterising the set of gradients of zero, and then considering the elements orthogonal to this set. \newline

Having introduced this general theory, sections \ref{sec:ScalarExample}-\ref{sec:ReduceToQMProblem} focus on singular-structure domains and the spaces that we will need for our work on variational problems involving singular measures.
...
These results on a bounded domain with a finite underlying graph $\graph$ coincide with the situation we find ourselves in for our singular-structure problems after taking a Gelfand transform (section \ref{sec:OurPhysicalSetup}).
...
Accounting for the $\qm$ still allows us to write tangential gradients edge-wise, but now these include terms involving $\qm$.
