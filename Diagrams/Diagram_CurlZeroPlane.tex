...
%			\draw (5,-1) to[in=290, out=65] (5,1) to[in=205, out=115] cycle;
		...
		\node[anchor=east, align=right] at (4,3) {Plane $P$ in \\ $\bracs{x_2,x_3}$-plane};
		\node[anchor=east, align=right] at (-1,2) {Axis vector \\ $\bracs{0,c_2,0}^\top$};
		\node[anchor=east, align=right] at (-1.25,7) {Axis vector \\ $\bracs{c_1,0,0}^\top$};
		\node[anchor=west, align=left] at (6,7) {Projection of (area \\ enclosed by) rotation};
		\node[anchor=west, align=left] at (6,2) {Projection of (area \\ enclosed by) rotation};
	\end{tikzpicture}
