...
\begin{tikzpicture}[->,>=stealth',auto,node distance=3cm,thick,main node/.style={circle,draw,font=\sffamily\Large\bfseries}]
	...
	\node[align=center] (ARROW) at (3.5, 3) {Simple analytic \\ models};
	\node[align=center] (Reson) at (0, 2.5) {Resonance between \\ fibre cores};
	\node[align=center] (Intermediate) at (-3.5,3) {Intermediate \\ approaches};
	
	\node[align=center] (GDC) at (-5,-8) {Gradient, curl, and \\ divergence analysis};
	%\node[align=center] (WaveMotivation) at (-5,-1.5) {Wave-guidence \\ motivation};
	
	\node[align=center] (MMat) at (5,-1.5) {Theory of \\ boundary triples};
	
	%edges between main topics
	\path[every node/.style={font=\sffamily\small}]
		(TS) edge node[anchor=north west, align=center] {Wave-guidance \\ motivation} (SS)
