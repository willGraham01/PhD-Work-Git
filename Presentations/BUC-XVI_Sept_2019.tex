...
\begin{frame}
		...
		\begin{column}{0.6\textwidth}
			...
			\only<4>{
				\begin{figure}
					...
					\caption{Spectrum of the whole-space problem (1st branch)}
				\end{figure}
			}
		\end{column}
	\end{columns}
	...
	\only<2>{
		...
		\begin{align*}
			M\bracs{\omega,\theta}\Gamma_0 u = \Gamma_1 u
		\end{align*}
		when $\Gamma_0 u$ is set to the $k^{\mathrm{th}}$ canonical unit vector, $\Gamma_1 u$ is the $k^{\mathrm{th}}$ column of $M\bracs{\omega, \theta}$.
	}
	\only<3>{
		We numerically solve the QG problem for the Dirichlet data $\Gamma_0 u = e_{k}$, and read off the Neumann data $\Gamma_1 u$ as the column of $M$. \newline
	...
	\only<4>{
		We can view the spectrum by creating plots like the above, and export the values to data-files.
	}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\only<2-3>{
			\item Enhancements to the numerical scheme and stability
			\begin{itemize}
				\item Alternative solver methods for eigenvalue problems
				\item Handling of the ``Fourier variable", which was omitted in the example
			\end{itemize}
		}
		\only<3>{
			\item Analysis: singular-structures as an asymptotic limit of thin-structures
			\begin{itemize}
				\item Strengthen the link between the two formulations
...
\begin{frame}
	...
	\begin{block}{References}
		\begin{itemize}
			\item physics papers for fibre stuff
			\item Zhikov papers,
			\item EKK, Kirill graphs, etc!
			%need to go back through an insert the refs in the slide text
		\end{itemize}
	\end{block}
\end{frame}
