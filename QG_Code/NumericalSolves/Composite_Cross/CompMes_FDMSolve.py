...
def FDM_ThetaLoop(nPts, fName, N=31, nEvals=3, alpha3=0., lOff=False, saveFuncs=False, dimSpecs=[False, 0, 0], sparseSolve=True):
    '''
    ...
    INPUTS:
        ...
        sparseSolve: bool, if True then sparse matrices will be used to construct FDM. Not recommended to be False unless system size is small.
    OUTPUTS:
    ...
    if dimSpecs[0]:
        ...
        if dimSpecs[1]==0:
            ...
            for i, t1 in enumerate(unifRange):
                ...
                evsSq, evecs, _ = FDM_FindEvals(N, theta, alpha3, lOff, nEvals=nEvals, checks=False, saveEvals=False, saveEvecs=False, sparseSolve=sparseSolve)
...
if __name__=='__main__':
    ...
    parser.add_argument('-noSparse', action='store_false', help='Do not use sparse matrices for FDM construciton. Not reccomended.')
    ...
    FDM_ThetaLoop(args.nPts, resultsFile, N=args.N, nEvals=args.nEvals, alpha3=args.a, lOff=args.lOff, saveFuncs=args.funcs, dimSpecs=dSpec, sparseSolve=args.noSparse)
